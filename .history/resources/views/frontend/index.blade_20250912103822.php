...
<!-- Explore By Collection Header -->
<div class="creative-collection-section py-5">
    <div class="container">
        <div class="text-center mb-5">
            <h2 class="creative-collection-heading">Explore Our Collections</h2>
            <p class="creative-collection-subheading">Discover curated pieces designed to elevate your everyday style.</p>
        </div>
        <div class="creative-collection-masonry">
            @foreach($collection as $index => $coll)
            <div class="creative-collection-card" style="animation-delay: {{ $index * 0.12 }}s;">
                <span class="creative-sparkle" style="top:12%;left:18%;animation-delay:{{ $index * 0.2 }}s"></span>
                <span class="creative-sparkle" style="bottom:18%;right:14%;animation-delay:{{ $index * 0.3 + 0.5 }}s"></span>
                <a href="{{ route('frontend.product_detail',['id'=>$coll->id]) }}" class="creative-collection-link">
                    <div class="creative-card-bg"></div>
                    <img src="{{ asset('images/product/'.$coll->image) }}" alt="{{ $coll->name }}" class="creative-card-img">
                    <div class="creative-card-info">
                        <h5>{{ $coll->name }}</h5>
                        <span class="creative-view-more">
                            View Collection
                            <svg width="22" height="22" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M5 12h12M13 8l4 4-4 4" />
                            </svg>
                        </span>
                    </div>
                    <div class="creative-card-shape"></div>
                </a>
            </div>
            @endforeach
        </div>
...
.creative-collection-card {
    position: relative;
    border-radius: 32px 32px 60px 32px;
    overflow: visible;
    min-height: 400px;
    box-shadow: 0 12px 36px rgba(88,30,30,0.13), 0 2px 12px rgba(248,186,75,0.10);
...
.creative-collection-card:nth-child(odd) { margin-top: 40px; }
.creative-collection-card:hover {
    transform: translateY(-22px) scale(1.06) rotate(-2deg);
    box-shadow: 0 24px 64px rgba(88,30,30,0.22), 0 4px 18px rgba(248,186,75,0.18);
    z-index: 4;
}
.creative-collection-link {
    display: block;
    position: relative;
    height: 100%;
    text-decoration: none;
...
.creative-card-bg {
    ...
    right: -24px;
    bottom: -24px;
    background: linear-gradient(135deg, #f8ba4b33 0%, #7a4c4c22 100%);
    border-radius: 40px;
    z-index: 0;
    filter: blur(12px);
    opacity: 0.8;
...
.creative-collection-card:hover .creative-card-bg { opacity: 1; }
.creative-card-img {
    width: 100%;
    height: 320px;
    object-fit: cover;
    border-radius: 28px 28px 60px 28px;
    box-shadow: 0 6px 32px rgba(88,30,30,0.13);
    position: relative;
    z-index: 2;
    transition: transform 0.6s cubic-bezier(.23,1.01,.32,1), filter 0.5s;
}
.creative-collection-card:hover .creative-card-img {
    transform: scale(1.11) rotate(2.5deg);
...
.creative-card-info {
    ...
    z-index: 3;
    color: #fff;
    text-shadow: 0 2px 16px rgba(88,30,30,0.22);
}
.creative-card-info h5 {
    font-size: 1.55rem;
    font-weight: 800;
    margin-bottom: 12px;
    font-family: 'Playfair Display', serif;
...
.creative-view-more {
    ...
    gap: 10px;
    font-size: 1.05rem;
    background: rgba(88,30,30,0.92);
    padding: 8px 22px 8px 16px;
    border-radius: 22px;
    font-weight: 600;
    box-shadow: 0 2px 12px rgba(248,186,75,0.13);
    transition: background 0.3s, color 0.3s;
}
.creative-collection-card:hover .creative-view-more {
    background: linear-gradient(90deg,#F8BA4B 60%,#fff7f0 100%);
    color: #581E1E;
}
.creative-view-more svg {
    margin-left: 4px;
    vertical-align: middle;
    transition: transform 0.3s, stroke 0.3s;
}
.creative-collection-card:hover .creative-view-more svg {
...
.creative-card-shape {
    ...
    width: 90px;
    height: 90px;
    background: radial-gradient(circle at 60% 40%, #F8BA4B 0%, #fff0 80%);
    border-radius: 50%;
    opacity: 0.32;
    z-index: 1;
    pointer-events: none;
...
.creative-collection-card:hover .creative-card-shape { opacity: 0.6; }
/* Sparkle Animation */
.creative-sparkle {
    position: absolute;
    width: 10px;
    ...
    opacity: 0.7;
    pointer-events: none;
    animation: sparkleMove 2.2s infinite;
    z-index: 5;
}
...
@media (max-width: 900px) {
    .creative-card-img { height: 200px; }
    .creative-card-info { left: 12px; bottom: 12px; }
}
@media (max-width: 600px) {
    .creative-collection-heading { font-size: 2rem; }
    .creative-card-img { height: 120px; }
    .creative-card-info h5 { font-size: 1rem; }
    .creative-collection-masonry { gap: 18px 8px; }
}
@keyframes fadeInUp {
