...
<div class="container-fluid py-4">
    <div class="row g-4 justify-content-center">
        ...
        @php
            // Extract sub images (pipe separated string → array)
            $subImages = $coll->images ? explode('|', $coll->images) : [];
            // Pick first sub image if exists, otherwise fallback to main image
            $hoverImage = count($subImages) > 0 ? $subImages[0] : $coll->image;
        @endphp
        ...
        <div class="col-lg-3 col-md-4 col-sm-6">
            <a href="{{ route('frontend.product_detail',['id'=>$coll->id]) }}" class="collection-link">
                <div class="collection-card">
                    ...
                    {{-- Hover image (first sub image if available) --}}
                    <img src="{{ asset('images/product/'.$hoverImage) }}" 
                         alt="{{ $coll->name }} hover" 
                         class="hover-img">
