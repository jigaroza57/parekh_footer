...
<br>
<!---Shop by Category-->
<!-- Explore By Collection - Unique Vertical Stack Design -->
<section class="collections-stack-section py-5">
    <div class="container">
        <div class="text-center mb-5">
            <h2 class="collections-stack-title">Explore Our Collections</h2>
            <p class="collections-stack-subtitle">Discover signature pieces, each with a story and a soul.</p>
        </div>
        <div class="collections-stack-wrapper">
            @foreach($collection as $coll)
            <a href="{{ route('frontend.product_detail',['id'=>$coll->id]) }}" class="stack-card" style="--i:{{ $loop->index }}">
                <div class="stack-card-bg" style="background-image: url('{{ asset('images/product/'.$coll->image) }}');"></div>
                <div class="stack-card-glass"></div>
                <div class="stack-card-content">
                    <h5>{{ $coll->name }}</h5>
                    <span class="stack-view-link">
...
.collections-stack-section {
    background: linear-gradient(135deg, #f3e8ff 0%, #fff7f0 100%);
    position: relative;
    overflow: hidden;
    min-height: 700px;
}
.collections-stack-title {
    font-family: 'Playfair Display', serif;
    font-size: 2.8rem;
    color: #4e1e6a;
    letter-spacing: 2px;
...
.collections-stack-subtitle {
    font-family: 'Montserrat', sans-serif;
    font-size: 1.15rem;
    color: #7a4c4c;
    margin-bottom: 2.5rem;
}
.collections-stack-wrapper {
    display: flex;
...
.stack-card {
    ...
    position: absolute;
    left: 50%;
    transform: translateX(-50%) translateY(calc(var(--i) * 40px)) scale(calc(1 - var(--i)*0.07));
...
.stack-card:hover {
    transform: translateX(-50%) translateY(-30px) scale(1.08) rotate(-2deg);
    box-shadow: 0 24px 64px rgba(90,45,130,0.22), 0 4px 24px rgba(255,179,71,0.18);
    z-index: 200;
}
...
.stack-card:hover .stack-card-glass {
    opacity: 0.95;
}
.stack-card:hover .stack-view-link {
    background: #ffb347;
    color: #4e1e6a;
}
@media (max-width: 600px) {
    .stack-card, .collections-stack-wrapper {
        width: 95vw;
        min-width: unset;
        max-width: unset;
    }
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    // Bring hovered card to top
    document.querySelectorAll('.stack-card').forEach(card => {
        card.addEventListener('mouseenter', function() {
            this.style.zIndex = 300;
        });
        card.addEventListener('mouseleave', function() {
            this.style.zIndex = 100 - this.style.getPropertyValue('--i');
        });
    });
});
</script>
